...
function App() {
  ...
  return (
    <main className="shell">
        ...
        <div className="history">
          {finishedMatches.map((match) => (
            <HistoryMatch
              ...
              players={data.players}
              disabled={busy}
              onEdit={() => setEditingFinishedMatch(match)}
            />
          ))}
      ...
      {editingFinishedMatch && (
        <EditFinishedMatchModal
          match={editingFinishedMatch}
          predictions={data.predictions.filter((prediction) => prediction.match_id === editingFinishedMatch.id)}
          players={data.players}
          disabled={busy}
          onResult={saveResult}
          onCancelResult={cancelResult}
          onPredictionUpdate={updateFinishedPrediction}
          onClose={() => setEditingFinishedMatch(null)}
        />
      )}
    </main>
  );
    ...
    <details className="history-row" open={open} onToggle={(event) => setOpen((event.currentTarget as HTMLDetailsElement).open)}>
      <summary className="history-summary">
        ...
        <div className="history-actions">
          ...
          <button type="button" className="secondary-button edit-button" disabled={disabled} onClick={(event) => { event.preventDefault(); onEdit(); }}>
            Edytuj
          </button>
        </div>
      </summary>
...
function EditFinishedMatchModal({
  match,
  predictions,
  players,
  disabled,
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal wide-modal" onClick={(event) => event.stopPropagation()}>
        <h2>Edytuj zakonczony mecz</h2>
        <div className="modal-form">
          <div className="form-field">
            <label>{match.home_team} - {match.away_team}</label>
            <div className="result-form modal-result-form">
              <span>Wynik meczu</span>
          ...
          <div className="modal-predictions">
            {players.map((player) => {
              ...
              if (!prediction) {
                return (
                  <div key={player.id} className="modal-prediction-row">
                    <strong>{player.name}</strong>
                    <span>brak typu</span>
                  </div>
                );
              }
              const draft = drafts[prediction.id] ?? { home: "", away: "" };
              return (
                <div key={player.id} className="modal-prediction-row">
                  <strong>{player.name}</strong>
